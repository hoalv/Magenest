...
namespace Magenest\OrderAttribute\Model\Sales\Order\Pdf;
...
class Invoice extends \Magento\Sales\Model\Order\Pdf\AbstractPdf
{
     ...
     * @return void
     */
    protected function _drawHeader(\Zend_Pdf_Page $page , $yAttr)
    {
        /* Add table head */
        ...
        $page->drawRectangle(25, $yAttr - 20 , 570, $yAttr - 35 );
        $this->y = $yAttr - 30  ;
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        $page->setLineColor(new \Zend_Pdf_Color_GrayScale(0.5));
        $page->setLineWidth(0.5);
        $page->drawRectangle(25, $top - 25 - $addressesHeight, 570, $top - 43 - $addressesHeight);

        $page->setFillColor(new \Zend_Pdf_Color_GrayScale(0));
//        $this->_setFontBold($page, 12);
        $page->drawText(__('Additional Info'), 35, $top - 38 - $addressesHeight, 'UTF-8');
        $this->_setFontRegular($page, 10);

        $yAttribute = $top - 58 - $addressesHeight;
        $attributes = $this->getOrderAttribute();
        $countryName = $this->getCountryNameByIp();

        if($attributes) {
            foreach ($attributes as $data) {
                if(($data->getCountryId()==null || $countryName == $this->getCountrynameByCode($data->getCountryId()) )
                    && $data->getIsVisibleOnBack() == 1 && $data->getIncludePdf() == 1) {
                    $option = $this->getOptionArrayById($data->getAttributeId());
                    $valueAttribute = $this->getOrderAttributeValue($order->getId(), $data->getAttributeId());
                    if ($valueAttribute != '') {
                        ...
                        //draw
                        $page->drawText(__($data->getFrontendLabel().':   '.$valueAttribute), 35, $yAttribute, 'UTF-8');

                    } else {
                        //draw
                        $val = $data->getDefaultValue()?$data->getDefaultValue():'No value';
                        $page->drawText(__($data->getFrontendLabel().':   '.$val), 35, $yAttribute, 'UTF-8');
                    }
            ...
            //draw
            $page->drawText(__('No attributes for this order'), 35, $yAttribute, 'UTF-8');
        }
